...
class InventoryActivateHandler(InventoryEventHandler):
    ...
    def on_item_selected(self, item: Item) -> Optional[ActionOrHandler]:
        if item.consumable:
            ...
            return actions.EquipAction(self.engine.player, True, item)
        else:
            return None
...
class InventoryDropHandler(InventoryEventHandler):
    ...
    def on_item_selected(self, item: Item) -> Optional[ActionOrHandler]:
        """Drop this item."""
        return actions.DropItem(self.engine.player, True, item)
...
class MainGameEventHandler(EventHandler):
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        if key in MOVE_KEYS:
            dx, dy = MOVE_KEYS[key]
            action = BumpAction(player, True, dx, dy)
        elif key in WAIT_KEYS:
            action = WaitAction(player, True)
            ...
            action = PickupAction(player, True)
