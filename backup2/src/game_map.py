...
class GameMap:
    def __init__(
        ...
        self.tiles = np.full( # tiles: What is actually there in the game world
            (width, height), fill_value=tile_types.white_wall, order="F"
            )
        self.tiles_memory = np.full( # tiles_memory: What the player remembers seeing / what is currently displayed
            (width, height), fill_value=tile_types.white_wall, order="F"
            )
    ...
    def render(self, console: Console) -> None:
        ...
        for entity in entities_sorted_for_rendering:
            if self.lit_and_visible[entity.x, entity.y]:
                ...
                console.print(
                    x=entity.x, y=entity.y, string='?', fg=(96,96,128)
                )
